...
| -------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------- | --------------- |
| NAMING_CONVENTION          | **Compliant** | snake_case/verb-first; notebooks follow header convention                                               | —              |
| FOLDER_STRUCTURE           | **Compliant** | Top-level layout matches; dead`load_cifar10.py` removed                                               | —              |
| MD_CONVENTION              | **Compliant** | Docs have header + TOC; cross-reference links mandatory                                                 | —              |
| NOTEBOOK_HEADER_CONVENTION | **Compliant** | All notebooks: title, subtitle, roadmap,`## References`; analysis-only                                | —              |
| AGENT_AI (docs/logging)    | **Compliant** | `agents/` phase/status/experiment docs maintained; LOGGING_CHECKPOINT_RULES.md + RESULTS_REPORTING.md | —              |
...
| Code quality             | **Good**   | Documented modules; duplicate loaders/SOTA builders consolidated                       |
| Security                 | **Good**   | `weights_only=True`; no shell/secrets issues                                         |
| Dependency health        | **Good**   | `requirements.lock` pins the matrix; unused deps pruned; 3.14 constraints documented |
| Architecture consistency | **Strong** | Single`data/raw` root, canonical loader, script-owned artifacts                      |
| Test coverage            | **Fair**   | Suite runs green (pytest.ini + CI); hot paths covered; data tests skip without data    |
| Performance              | **Fair**   | Feature caches; shared raw dataset;`num_workers` trade-off documented                |

Evidence for each dimension is in [§3](#3-code-quality)–[§8](#8-performance-bottlenecks); resolution log in [§13](#13-remediation-status--resolution-log).
...
## 13. Remediation Status & Resolution Log

> Living log updated as remediation lands. All action items below are now
> resolved unless noted; the baseline findings in §2–§12 remain for history.

| ID     | Status      | Evidence (landed in this repo)                                                                                                            |
| ------ | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| SEC-1  | ✅ Resolved | `weights_only=True` on every `torch.load`; notebooks load via `checkpoint_utils.load_model_weights`                                 |
| DEP-1  | ✅ Resolved | `requirements.lock` (pip freeze, 138 pinned packages) added                                                                             |
| DEP-2  | ✅ Resolved | `pytest` installed; `opencv-python` pruned (verified unused in code + notebooks)                                                      |
| DEP-3  | ✅ Resolved | `tabulate`, `tqdm` removed from `requirements.txt` (unused everywhere)                                                              |
| DEP-4  | ✅ Resolved | Exact matrix pinned in`requirements.lock`; Python 3.14 `num_workers` constraint documented                                            |
| TST-1  | ✅ Resolved | `pytest` installed; `pytest.ini` added; suite runs green                                                                              |
| TST-2  | ✅ Resolved | New hot-path tests: split, loaders, build_model modes, train smoke + exact resume, evaluate, config, feature_extraction, checkpoint_utils |
| TST-3  | ✅ Resolved | `.github/workflows/ci.yml` (pytest + ruff on push/PR)                                                                                   |
| TST-4  | ✅ Resolved | Synthetic fixtures;`tests/conftest.py` skips data-dependent tests when `data/raw` absent                                              |
| ARC-1  | ✅ Resolved | Single`data/raw` root everywhere; `data/external` retired; regression test (`test_config`)                                          |
| ARC-2  | ✅ Resolved | Canonical`dataloader.py`; dead `load_cifar10.py` deleted; imports swept                                                               |
| ARC-3  | ✅ Resolved | Docs swept for`docs/`, `data/external`, stale notebook refs                                                                           |
| ARC-4  | ✅ Resolved | `experiments/results/**/*.{npz,pt}` gitignored + untracked; policy documented in [§6](#6-architecture-consistency)                      |
| CQ-1   | ✅ Resolved | Duplicate loader deleted;`dataset.py` routed to `data/raw`                                                                            |
| CQ-2   | ✅ Resolved | `dataloader.py` consumes `configs/data.yaml` defaults (batch/workers/root)                                                            |
| CQ-3   | ✅ Resolved | `src/scratch/` retired                                                                                                                  |
| CQ-4   | ✅ Resolved | `build_resnet18_full_sota`/`build_densenet121_full_sota` + LLRD groups in `src/models/build_model.py`                               |
| CQ-5   | ✅ Resolved | `pyproject.toml` (ruff/mypy/pytest); ruff enforced in CI                                                                                |
| PERF-1 | ✅ Resolved | Trade-off documented (README + dataloader +`data.yaml`); workers remain 0 by default on 3.14                                            |
| PERF-2 | ✅ Resolved | Feature extraction scripted; NPZ caches persist between runs (gitignored, regenerable)                                                    |
| PERF-3 | ✅ Resolved | Loaders build 2 raw instances (train shared by train/val via`_ApplyTransform`) instead of 3                                             |
| PERF-4 | ✅ Resolved | Notebooks are analysis-only; no training loops; artifacts loaded                                                                          |

**Notes:** No open findings remain. The small meta-model `.pt` state dicts and
feature NPZ caches are intentionally gitignored and regenerated by
`src/experiments/*_train.py` per the artifact-storage policy ([ARC-4](#arc-4-binary-artifacts-pt--npz-risk-repo-bloat)).
