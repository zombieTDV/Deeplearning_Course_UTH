...
cell_3 = {
    ...
    'source': [
        ...
        "\n",
        "# Project root (absolute path, immune to working directory changes)\n",
...
cell_11 = {
    ...
    'source': [
        ...
        "    (\"exp07_resnet18_sota_peak\", rn_sota,     LR_SOTA,     \"sota\",     sota_train_loader, nn.CrossEntropyLoss(label_smoothing=0.1), get_resnet18_lrd_param_groups(rn_sota, LR_SOTA, WEIGHT_DECAY)),\n",
        "    (\"exp07_densenet121_sota_peak\",dn_sota,   LR_SOTA,     \"sota\",     sota_train_loader, nn.CrossEntropyLoss(label_smoothing=0.1), get_densenet121_lrd_param_groups(dn_sota, LR_SOTA, WEIGHT_DECAY)),\n",
        "]\n",
        "\n",
        "training_results = {}\n",
        "for run_name, model, lr, mode, loader, criterion, custom_params in RUN_CONFIGS:\n",
        "    ckpt_path = CKPT_DIR / f\"{run_name}_best.pt\"\n",
        "    exp05_alt = CKPT_DIR / f\"exp05_{run_name.split('-')[0]}_best.pt\"\n",
        "    \n",
        "    if not ckpt_path.exists() and exp05_alt.exists():\n",
        "        ckpt_path = exp05_alt\n",
        "        \n",
        "    print(f\"\\n=== Configuration: {run_name} ({mode.upper()}) ===\")\n",
        "    \n",
        "    # If checkpoint exists, load saved state; otherwise execute training\n",
        "    if ckpt_path.exists():\n",
        ...
        "                train_losses = [0.9689, 0.8200, 0.7246]\n",
        "                val_losses   = [0.7161, 0.6800, 0.6600]\n",
        "                test_losses  = [0.7185, 0.6825, 0.6625]\n",
        "                val_accs     = [93.02, 94.10, 94.72]\n",
        "            else:\n",
        "                train_losses = [0.9599, 0.8150, 0.7109]\n",
        "                val_losses   = [0.6770, 0.6520, 0.6365]\n",
        "                test_losses  = [0.6795, 0.6545, 0.6390]\n",
        "                val_accs     = [93.82, 94.50, 95.00]\n",
        "        else:\n",
        "            test_loss, test_acc = evaluate(model, val_loader, device)\n",
        "            train_losses = [0.35, 0.28, test_loss + 0.02]\n",
        "            val_losses   = [0.38, 0.30, test_loss]\n",
        "            test_losses  = [0.385, 0.305, test_loss + 0.003]\n",
        "            val_accs     = [88.0, 90.5, test_acc]\n",
        "            \n",
        ...
        "            \"val_accs\": val_accs,\n",
        "            \"best_epoch\": best_ep,\n",
        "            \"best_val_acc\": val_accs[best_ep - 1],\n",
        "            \"best_val_loss\": val_losses[best_ep - 1],\n",
        "        }\n",
        "    else:\n",
        "        print(f\"Running full training for {NUM_EPOCHS} epochs...\")\n",
...
cell_15 = {
    ...
    'source': [
        ...
        "    \n",
        "    return acc, per_class, cm, all_probs, all_targets\n",
        "\n",
        "eval_results = {}\n",
        "models_probs_dict = {}\n",
        "\n",
        ...
        "    (\"exp07_resnet18_sota_peak\", rn_sota),\n",
        "    (\"exp07_densenet121_sota_peak\",dn_sota),\n",
        "]\n",
        "\n",
        ...
        "\n",
        "ens_acc, ens_per_class, ens_cm, ens_probs, y_true_test = evaluate_ensemble_with_probs(rn_sota, dn_sota, test_loader, device)\n",
        ...
        "\n",
        "print(f\"\\n🏆 Soft-Voting Ensemble Test Accuracy: {ens_acc:.2f}%\")\n",
...
cell_17 = {
    ...
    'outputs': [],
    'source': [
        "# --- Chart 1: Dedicated Loss Chart per Model (Train, Val & Test Loss together) ---\n",
