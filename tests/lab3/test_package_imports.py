...
def test_src_package_imports_cleanly():
    # B1: importing the src.lab3 package must not eagerly import heavy/optional
    # helpers (transformers, IPython, scratch.analyze_misclassifications).
    import src.lab3 as src_lab3

    assert hasattr(src_lab3, "__getattr__")  # lazy export mechanism active
...
def test_src_lazy_exports_resolve():
    ...
    for name in _LAZY_NAMES:
        assert getattr(src_lab3, name) is not None, f"lazy export {name} failed"
...
def test_error_auditor_imports_without_scratch():
    ...
    mod = importlib.import_module("src.lab3.eval.error_auditor")
...
def test_training_module_imports_and_exposes_resume_helper():
    ...
    mod = importlib.import_module("src.lab3.training.imdb_sentiment_train")
