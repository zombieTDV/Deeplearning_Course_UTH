...
- **Overview Pipeline**: Derived from the LAB2 workflow: conventional commits →
...
- **scope** (optional): the affected area, e.g. `LAB2`, `train_lab2_models`,
...
Examples:
- `fix(LAB2): apply finetune mode so layer4 is actually trained`
- `feat(train_lab2_models): add --force-resume to rewind to the best epoch`
...
### 2.6 Good vs bad
- Good: `fix(LAB2): raise finetune base LR to 1e-3 so head trains`
...
- **Never commit large binaries to git.** Model weights, feature caches, and
  ...
  artifact-storage policy in [codebase-audit-report.md](../codebase-audit-report.md).)
