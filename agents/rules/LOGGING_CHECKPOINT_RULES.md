...
- **Purpose**: Define the authoritative, **project-generic** rules for log files, checkpoints, directory layout, naming, the load/resume procedure, and automatic output persistence. They apply to every feature project in this repository (LAB1, LAB2, LAB3, …). Feature-specific examples live in the optional [Appendix A](#appendix-a--optional-feature-specific-example-lab2).
...
- **Detailed Plan**: §1 policy (scripts vs notebooks, automatic persistence); §2 directory structure; §3 checkpoint file format; §4 naming rules; §5 resume procedure; §6 metrics storage & compression; §7 log levels & real-time monitoring; §8 acceptance checklist; Appendix A (optional) feature-specific example.
...
- [Appendix A (optional) — Feature-Specific Example: LAB2](#appendix-a-optional--feature-specific-example-lab2)
...
## Appendix A (optional) — Feature-Specific Example: LAB2

> This appendix is a worked example for the LAB2 feature only. Other feature
> projects keep the same rules with their own script entry points, run names,
> and result directories. Do not copy feature names into the core rules.

- **Deliverable training script**: `python -m src.training.train_lab2_models`
  (`--modes frozen|finetune|sota`, `--epochs`, `--seed`, `--resume`, `--tb`, `--smoke`).
- **Meta-model scripts**: `python -m src.experiments.stacking_mlp_train`,
  `python -m src.experiments.moe_router_train`.
- **Run names**: `ResNet18-frozen`, `DenseNet121-frozen`, `ResNet18-finetune`,
  `DenseNet121-finetune`, `ResNet18-sota`, `DenseNet121-sota`, `stacking_mlp`, `moe_router`.
- **Consolidated outputs (script-written)**: `results/training_history.json`,
  `results/stacking_mlp/` (NPZ + state dicts + config), `results/moe_phase1/`.
- **Notebooks** (`notebooks/practice_2*.ipynb`) load these artifacts for
  analysis only.
