...
| External Model/Data Links | **Mandatory** — any model ID or dataset ID mentioned MUST include a direct clickable markdown link                                                                                                 |
| File paths                | Relative link (e.g. `docs/lab2/OVERVIEW.md`: internal), Cross-file anchor link (e.g. `docs/shared/HOW_TO_SETUP_AI_AGENT.md#step-1`: jump to a heading), Absolute URL (external) — never bare text |
