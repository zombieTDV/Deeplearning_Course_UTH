#!/usr/bin/env python3
"""
DF40 Dataset Comprehensive Analysis Script
...
class DF40Analyzer:
    def __init__(self):
        ...
        self.video_metadata = {}
        self.method_stats = defaultdict(dict)
        self.quality_issues = []
        self.duplicates = []
    ...
    def run_full_analysis(self):
        ...
        if self.quality_issues:
            ...
            quality_df.to_csv(self.metadata_root / 'quality_report.csv', index=False)
            print(f"Found {len(self.quality_issues)} quality issues")
        else:
            print("No quality issues found")
            pd.DataFrame(columns=['file_path', 'issue_type', 'error_message', 'is_trainable']).to_csv(self.metadata_root / 'quality_report.csv', index=False)
