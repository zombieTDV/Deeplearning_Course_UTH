#!/usr/bin/env python3
"""
DF40 Dataset Split Creation Script (Fixed)
...
def create_video_level_splits():
    ...
    # Create video_id to split mapping
    video_to_split = {}
...
def check_duplicate_leakage(video_to_split):
    ...
    # Create unique video IDs for samples
    samples_df['unique_video_id'] = samples_df['manipulation'] + '_' + samples_df['video_id'].astype(str)
    
    # Create hash to video mapping
    hash_to_video_ids = defaultdict(set)
    for _, row in samples_df.iterrows():
        file_hash = row.get('sha256', '')
        unique_video_id = row.get('unique_video_id', '')
        if file_hash and unique_video_id:
            hash_to_video_ids[file_hash].add(unique_video_id)
    
    # Check for duplicates that cross splits at video level
    cross_split_duplicates = 0
    cross_split_hashes = []
    
    for file_hash, video_ids in hash_to_video_ids.items():
    ...
    if cross_split_duplicates > 0:
        ...
        for file_hash in cross_split_hashes:
            ...
            unique_videos = dup_samples['unique_video_id'].unique().tolist()
            splits = [video_to_split.get(v, 'unknown') for v in unique_videos]
            cross_split_dup_info.append({
                'sha256': file_hash,
                'unique_videos': ','.join(unique_videos),
                'splits': ','.join(set(splits)),
...
if __name__ == "__main__":
    ...
    leakage_results = check_duplicate_leakage(split_results['video_to_split'])
