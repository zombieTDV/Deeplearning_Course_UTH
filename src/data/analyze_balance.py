#!/usr/bin/env python3
"""
DF40 Dataset Balance Analysis and Visualization Script
Analyzes class and manipulation balance and generates EDA visualizations.
"""
...
FIGURES_ROOT = METADATA_ROOT / "figures"

def analyze_balance():
    """Analyze class and manipulation balance in detail."""
    print("Analyzing class and manipulation balance...")
    ...
    for split in ['train', 'validation', 'test']:
        ...
        if len(split_df) > 0:
            split_class_counts = split_df['class_name'].value_counts()
            split_total = len(split_df)
            print(f"\n{split} ({split_total} samples):")
            for class_name, count in split_class_counts.items():
                print(f"  {class_name}: {count} ({count/split_total*100:.2f}%)")
    
    # Manipulation method balance
    print("\nManipulation Method Balance (Overall):")
    manipulation_counts = df['manipulation'].value_counts()
    for method, count in manipulation_counts.head(20).items():
        print(f"  {method}: {count} ({count/total*100:.2f}%)")
    
    # Manipulation balance by split
    print("\nManipulation Method Balance by Split:")
    for split in ['train', 'validation', 'test']:
        split_df = df[df['split'] == split]
        if len(split_df) > 0:
            split_manip_counts = split_df['manipulation'].value_counts()
            print(f"\n{split} (Top 10 methods):")
            for method, count in split_manip_counts.head(10).items():
                print(f"  {method}: {count}")
    
    # Fake class manipulation breakdown
    fake_df = df[df['class_name'] == 'FAKE']
    print(f"\nFAKE Class Manipulation Breakdown (Top 20):")
    fake_manip_counts = fake_df['manipulation'].value_counts()
    fake_total = len(fake_df)
    for method, count in fake_manip_counts.head(20).items():
        print(f"  {method}: {count} ({count/fake_total*100:.2f}%)")
    
    return {
        'overall_class_balance': dict(class_counts),
        'manipulation_balance': dict(manipulation_counts)
    }
...
if __name__ == "__main__":
    # Analyze balance
    balance_results = analyze_balance()
