...
from src.lab3.utils.run_logger import RunLogger
...
def main() -> None:
    ...
    if args.baseline or "baseline" in t.get("run_name", "").lower() or t.get("epochs", 4) == 0:
        ...
        print("=" * 65)
        import src.lab3.experiments.baseline_imdb_sentiment as base_exp
        device_id = 0 if torch.cuda.is_available() else -1
        base_exp.evaluate_zero_shot_imdb(
            model_name=config["model"]["name"],
            data_dir=Path(config["data"]["processed_dir"]),
            device_id=device_id,
            output_json=Path("experiments/lab3/results/baseline_imdb_sentiment.json"),
        )
        return
